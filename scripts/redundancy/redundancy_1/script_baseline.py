    ...
    for fs_method in fs_methods:
        ...
        for j, (train_index, test_index) in enumerate(rskf.split(raw_data, raw_label)):
            ...
            for i, n_features in enumerate([5, 10, 15, 20]):
                ...
                n_svc_aucs = []
                n_train_accuracies = []
                n_real_feats = [(fs_class.ranking[:n_features] < redundancy_1.REAL_FEATURES).sum()]
                n_redundant_feats = [(fs_class.ranking[:n_features] <
                                      (redundancy_1.REAL_FEATURES + redundancy_1.REDUNDANT_FEATURES)).sum() - n_real_feats[-1]]
                n_real_feats[-1] /= redundancy_1.REAL_FEATURES
                n_redundant_feats[-1] /= redundancy_1.REDUNDANT_FEATURES
