...
import numpy as np
import os
from dataset_reader import redundancy_2
...
normalization_func = redundancy_2.Normalize

dataset_name = 'redundancy_2'
...
def load_dataset():
    dataset = redundancy_2.load_dataset()
    return dataset
    ...
    for fs_method in fs_methods:
        ...
        for j, (train_index, test_index) in enumerate(rskf.split(raw_data, raw_label)):
            ...
            for i, n_features in enumerate([5, 10, 15, 20]):
                ...
                n_svc_aucs = []
                n_train_accuracies = []
                n_real_feats = [(fs_class.ranking[:n_features] < redundancy_2.REAL_FEATURES).sum()]
                n_redundant_feats = [(fs_class.ranking[:n_features] <
                                      (redundancy_2.REAL_FEATURES + redundancy_2.REDUNDANT_FEATURES)).sum() - n_real_feats[-1]]
                n_real_feats[-1] /= redundancy_2.REAL_FEATURES
                n_redundant_feats[-1] /= redundancy_2.REDUNDANT_FEATURES
        ...
        info_data = {
            ...
            'classification': {
                ...
                'mean_real_feats': np.mean(real_feats),
                'redundant_feats': redundant_feats,
                'mean_redundant_feats': np.mean(redundant_feats),
            }
        }
