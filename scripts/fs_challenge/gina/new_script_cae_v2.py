...
class ConcreteAutoencoderFeatureSelector():
    ...
    def fit(self, X, Y=None, val_X=None, val_Y=None, num_epochs=300, batch_size=None, start_temp=10.0,
        ...
        for i in range(tryout_limit):
            ...
            hist = self.model.fit(X, Y, batch_size, num_epochs,
                                  verbose=0,
                                  callbacks=[
                                      stopper_callback,
                                      callbacks.LearningRateScheduler(scheduler(max(0, num_epochs - 150)))
                                  ],
...
def main(dataset_name):
    ...
    output_filename = directory + 'LinearSVC_' + kernel + '_CAEv2.json'
