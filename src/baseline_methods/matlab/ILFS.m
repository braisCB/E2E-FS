...
for ii = 1 : params.iter
    %disp('E-step');
    for d = 1:numTokens
        for w = find(tokenFeatMatrix(d,:))
            for z = 1:numFactors
                prob_factor_token_feat{z}(d,w) = prob_token_factor(d,z) * prob_factor_feat(z,w) * ...
                    tokenFeatMatrix(d,w) / prob_token_feat(d, w);
            end
        end
    end
    ...
    %disp('M-step');
    %disp('update p(z|d)')
    concat = cat(2, prob_factor_token_feat{:}); % make n_p_z_dw{:}(d,:)) possible
    for d = 1:numTokens
        for z = 1:numFactors
            prob_token_factor(d,z) = sum(prob_factor_token_feat{z}(d,:));
        end
        prob_token_factor(d,:) = prob_token_factor(d,:) / sum(concat(d,:));
    end

    %disp('update p(w|z)')
    for z = 1:numFactors
        for w = 1:numFeat
            prob_factor_feat(z, w) = sum(prob_factor_token_feat{z}(:,w));
        end
        prob_factor_feat(z,:) = prob_factor_feat(z,:) / sum(prob_factor_token_feat{z}(:));
    end
    ...
    for d = 1:numTokens
        for w = find(tokenFeatMatrix(d,:))
            prob_token_feat(d,w) = 0;
            for z = 1:numFactors
                prob_token_feat(d,w) = prob_token_feat(d,w) + prob_factor_feat(z,w) * prob_token_factor(d,z);
            end
            L = L + tokenFeatMatrix(d,w) * log(prob_token_feat(d, w));
        end
    end
